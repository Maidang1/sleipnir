...
//! `agent_coordination::Registry`, the grant-gated Unix socket server bound
...
use crate::adapter::{Adapter, HostCalls, missing_delivery_grants};
use crate::state::AgentState;
use crate::view::{exit_notice, panel_tree, status_tree};
...
impl Agents {
    ...
    fn start_coordination(&mut self, granted: &[Capability]) {
        ...
        // Worker self-report commands need the same override as this server.
    ...
    fn on_event(&mut self, event: HostEvent, ctx: &mut Context<'_>) {
        match event {
            ...
            HostEvent::RunFinished { run_id, pane, .. } => {
                ...
                if matches!(outcome, crate::state::RunFinishOutcome::Exited { .. }) {
...
/// Run the standalone plugin over stdin/stdout.
pub fn run_plugin() {
    run(Agents::new());
}

/// Run the copy compiled into the terminal, still in an isolated process.
pub fn run_builtin() {
    let mut agents = Agents::new();
    if let Ok(executable) = std::env::current_exe() {
        agents.adapter.set_builtin_executable(&executable);
    }
    run(agents);
}
...
mod tests {
    ...
    #[test]
    fn compiled_manifest_matches_the_resident_protocol_audit() {
        let plugin = Agents::new();
        let manifest: plugin_host::PluginManifest =
            serde_json::from_str(include_str!("../plugin.json")).unwrap();
        let requests: std::collections::BTreeSet<_> = plugin.requests().into_iter().collect();
        assert_eq!(plugin.manifest().id, manifest.id);
        assert_eq!(plugin.manifest().version, manifest.version);
        assert_eq!(plugin.manifest().lifecycle, Lifecycle::Resident);
        assert_eq!(
            requests,
            plugin_host::resident::declared_capabilities(&manifest)
        );
        assert!(missing_delivery_grants(&plugin.requests()).is_empty());
    }
}
