...
pub fn launch_application(bundle: &Path) -> Result<u32, String> {
    ...
    let path = bundle.to_str().ok_or_else(|| "application path is not UTF-8".to_string())?;
    ...
    let handler = RcBlock::new(move |application: *mut NSRunningApplication, error: *mut NSError| {
        if !error.is_null() {
            let _ = sender.send(Err("LaunchServices failed to start candidate".to_string()));
        } else if application.is_null() {
            let _ = sender.send(Err("LaunchServices returned no running application".to_string()));
        } else {
            // SAFETY: NSWorkspace owns the callback object for the callback duration.
            let pid = unsafe { (&*application).processIdentifier() };
            if pid > 0 { let _ = sender.send(Ok(pid as u32)); }
            else { let _ = sender.send(Err("candidate has no process identifier".to_string())); }
        }
    });
    ...
    receiver.recv_timeout(Duration::from_secs(15)).map_err(|_| "LaunchServices callback timed out".to_string())?
}
