...
//! (the schema already defined in v2) and `syntax` (code highlighting, matching
//! the diff inspector — ADR-0012). It does **not** depend on gpui, does not
//! name `Hsla` or `Pixels`, and does no `f32` layout math. Sizes are integer
//! cells so a Block's height is an integer row count (ADR-0018) and survives
//! font-size changes.
//!
...
pub use cells::{
    ...
    cell_cols, language_for_widget, wrap_text,
};
