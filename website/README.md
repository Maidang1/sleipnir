...
Copy tracks the shipped product surface in the monorepo README (M0–M15): GPU terminal (Metal on macOS, Direct3D on Windows, Vulkan on Linux), tabs/splits/pane zoom, multi-window, themes, smart paste, session restore, path links, shell prompt jump, and optional Quick Terminal / opacity. Prebuilt downloads: macOS .dmg/.zip, Windows x64 .zip, and Linux .deb / .tar.gz from GitHub Releases.
