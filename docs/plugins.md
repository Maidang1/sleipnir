...
processes and talks to over a versioned RPC. A plugin runs
...
Sleipnir supplies context and routing; the plugin (and whatever it shells out
to) supplies behavior. Sleipnir makes no model calls and stores no provider
credentials.
...
acceptance window covers only dialects the host still
...
`sep`) produce the closed widget set. Colour is semantic [`Tone`] only — no
hex, no RGB.
...
| --- | --- | --- |
| `RenderTarget::Block { anchor: RunId }` | `render_block` | Inside scrollback, anchored to that run |
...
The renderer draws it; a crafted tree cannot hide it.
