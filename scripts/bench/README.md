# Sleipnir 性能基准 runbook

配合 [`docs/perf-baseline.md`](../../docs/perf-baseline.md) 使用。这里只讲「怎么跑」，方法/口径/竞品参照见那个文档。
...
再在对照终端（Terminal.app / iTerm2 / kitty / Ghostty 任选）跑同样的命令。把三个数字（real/user/sys）抄进 `docs/perf-baseline.md` 的 B1 表。
...
> 注意：全行业**没有**任何一手的启动 benchmark（见 perf-baseline.md B5），这里测出来本身就是填补空白的数字。
