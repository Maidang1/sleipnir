...
pub trait Handler {
    ...
    /// Unhandled OSC command (Sleipnir patch: routes OSC 133/9/777 to the
    /// terminal emulator, which may act on them). `code` is the first
    /// parameter (e.g. `b"133"`); `params` are all `;`-split payload parts.
    fn osc_custom(&mut self, _code: &[u8], _params: &[&[u8]]) {}
    ...
    T: Timeout,
{
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        match params[0] {
            ...
            // Sleipnir patch: route unhandled OSC (133 / 9 / 777, …) to the
            // handler so the terminal can act on shell-integration sequences.
            _ => {
                self.handler.osc_custom(params[0], params);
                unhandled(params);
            }
        }
    }
