...
pub enum Event {
    ...
    /// Shell-integration marker (`ESC ] 133 ; <kind>`), kind like `"A"`, `"B"`,
    /// `"C"`, or `"D;0"`. Sleipnir patch (ADR-0005).
    Osc133(String),

    /// Desktop notification (`ESC ] 9 ; <msg>` or `ESC ] 777 ; notify ; <msg>`).
    /// Sleipnir patch (ADR-0005).
...
impl Debug for Event {
    fn fmt(&self, f: &mut Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Event::Osc133(kind) => write!(f, "Osc133({kind})"),
            Event::DesktopNotification(msg) => write!(f, "DesktopNotification({msg})"),
        }
    }
}
