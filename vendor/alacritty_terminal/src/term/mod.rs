...
impl<T: EventListener> Handler for Term<T> {
    /// Sleipnir patch (ADR-0005): surface shell-integration OSC sequences that
    /// upstream drops. `params` are the OSC payload split on `;` (e.g. for
    /// `ESC ] 133;D;0 ST` → `[b"133", b"D", b"0"]`).
    fn osc_custom(&mut self, code: &[u8], params: &[&[u8]]) {
        match code {
            b"133" => {
                if let Some(kind) = params.get(1).and_then(|p| std::str::from_utf8(p).ok()) {
                    let payload = if kind == "D" {
                        let status = params
                            .get(2)
                    ...
                    self.event_proxy.send_event(Event::Osc133(payload));
                }
            }
            b"9" => {
                if let Some(msg) = params.get(1).and_then(|p| std::str::from_utf8(p).ok()) {
                    self.event_proxy
                        .send_event(Event::DesktopNotification(msg.to_string()));
                }
            }
            b"777" => {
                if params.get(1).and_then(|p| std::str::from_utf8(p).ok()) == Some("notify") {
                    let msg = params[2..]
                        .iter()
                        ...
                        .join(";");
                    self.event_proxy
                        .send_event(Event::DesktopNotification(msg));
                }
            }
            _ => {}
        }
...
mod tests {
    ...
    /// Collects `Event`s from `Term` so OSC 133/9/777 can be asserted.
    ...
    fn drain_osc_events(listener: &CollectingListener) -> (Vec<String>, Vec<String>) {
        let events = listener.events.lock().unwrap();
        let mut osc133 = Vec::new();
        let mut notify = Vec::new();
        for event in events.iter() {
            match event {
                Event::Osc133(kind) => osc133.push(kind.clone()),
                Event::DesktopNotification(msg) => notify.push(msg.clone()),
                _ => {}
            }
        }
        (osc133, notify)
    ...
    #[test]
    fn osc_custom_emits_133_and_desktop_notifications() {
        ...
        processor.advance(&mut term, b"\x1b]133;D;0\x07");
        processor.advance(&mut term, b"\x1b]9;hello\x07");
        processor.advance(&mut term, b"\x1b]777;notify;deploy;done\x07");
        // Unrelated OSC must not emit our events.
        processor.advance(&mut term, b"\x1b]0;title\x07");

        let (osc133, notify) = drain_osc_events(&listener);
        assert_eq!(osc133, ["A", "D;0"]);
        assert_eq!(notify, ["hello", "deploy;done"]);
    }

    #[test]
    fn osc_custom_accepts_st_terminator() {
        let listener = CollectingListener::default();
        let size = TermSize::new(80, 24);
        let mut term = Term::new(Config::default(), &size, listener.clone());
        let mut processor = ansi::Processor::<ansi::StdSyncHandler>::new();
        processor.advance(&mut term, b"\x1b]133;B\x1b\\");
